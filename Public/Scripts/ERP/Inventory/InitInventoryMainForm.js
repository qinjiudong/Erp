// 库存建账 - 主界面
Ext.define("ERP.Inventory.InitInventoryMainForm", {
    extend: "Ext.panel.Panel",
    initComponent: function () {
        var me = this;

        Ext.define("ERPInitInv", {
        ...
        var store = Ext.create("Ext.data.Store", {
            ...
            proxy: {
                ...
                url: ERP.Const.BASE_URL + "Home/InitInventory/initInfoList",
                reader: {
                    root: 'initInfoList',
        ...
        Ext.apply(me, {
            ...
            layout: "border",
            tbar: [{
                    text: "建账", iconCls: "ERP-button-add", scope: me, handler: me.onInitInv
                }, "-", {
                    text: "刷新", iconCls: "ERP-button-refresh", scope: me, handler: function () {
                        me.freshInvGrid();
                    }
                }, "-",
                {
                    text: "标记建账完毕", iconCls: "ERP-button-commit", scope: me, handler: me.onFinish
                },
                {
                    text: "取消建账完毕标记", iconCls: "ERP-button-cancel", scope: me, handler: me.onCancel
                }, "-",
                {
                    text: "帮助",
                    iconCls: "ERP-help",
                    handler: function() {
                        window.open("http://my.jyshop.net/u/134395/blog/375000");
                    }
                },
                "-", {
    ...
    freshWarehouseGrid: function () {
        ...
        Ext.Ajax.request({
            url: ERP.Const.BASE_URL + "Home/InitInventory/warehouseList",
            method: "POST",
    ...
    onInitInv: function () {
        ...
        if (item == null || item.length != 1) {
            ERP.MsgBox.showInfo("请选择要建账的仓库");
            return;
        }
        var warehouse = item[0];

        if (warehouse.get("inited") == 1) {
            ERP.MsgBox.showInfo("仓库[" + warehouse.get("name") + "]已经建账完毕");
            return;
        }

        var form = Ext.create("ERP.Inventory.InitInventoryEditForm", {
            warehouse: warehouse
        });
        form.show();
    },
    onFinish: function () {
        ...
        var item = this.gridWarehouse.getSelectionModel().getSelection();
        if (item == null || item.length != 1) {
            ERP.MsgBox.showInfo("请选择要标记的仓库");
            return;
        }
        var warehouse = item[0];
        if (warehouse.get("inited") == 1) {
            ERP.MsgBox.showInfo("仓库[" + warehouse.get("name") + "]已经标记建账完毕");
            return;
        }

        ERP.MsgBox.confirm("请确认是否给仓库[" + warehouse.get("name") + "]标记建账完毕?", function () {
            var el = Ext.getBody();
            el.mask(ERP.Const.SAVING);
            Ext.Ajax.request({
                url: ERP.Const.BASE_URL + "Home/InitInventory/finish",
                ...
                callback: function (options, success, response) {
                    ...
                    if (success) {
                        var data = Ext.JSON.decode(response.responseText);
                        if (data.success) {
                            ERP.MsgBox.showInfo("成功标记建账完毕", function () {
                                me.freshWarehouseGrid();
                            });
                        } else {
    ...
    onCancel: function () {
        ...
        var item = this.gridWarehouse.getSelectionModel().getSelection();
        if (item == null || item.length != 1) {
            ERP.MsgBox.showInfo("请选择要取消建账的仓库");
            return;
        }
        var warehouse = item[0];

        if (warehouse.get("inited") == 0) {
            ERP.MsgBox.showInfo("仓库[" + warehouse.get("name") + "]没有标记建账完毕");
            return;
        }

        ERP.MsgBox.confirm("请确认是否取消仓库[" + warehouse.get("name") + "]的建账完毕标志?", function () {
            var el = Ext.getBody();
            el.mask(ERP.Const.SAVING);
            Ext.Ajax.request({
                url: ERP.Const.BASE_URL + "Home/InitInventory/cancel",
                params: {warehouseId: warehouse.get("id")},
                method: "POST",
                callback: function (options, success, response) {
                    el.unmask();
                    if (success) {
                        var data = Ext.JSON.decode(response.responseText);
                        if (data.success) {
                            ERP.MsgBox.showInfo("成功取消建账标志", function () {
                                me.freshWarehouseGrid();
                            });
                        } else {
                            ERP.MsgBox.showInfo(data.msg);
