Ext.define("ERP.Home.MainForm", {
    ...
    getPurchaseGrid: function() {
    	...
    	var modelName = "ERPPortalPurchase";
        Ext.define(modelName, {
            extend: "Ext.data.Model",
    ...
    getInventoryGrid: function() {
    	...
    	var modelName = "ERPPortalInventory";
        Ext.define(modelName, {
            extend: "Ext.data.Model",
            fields: ["warehouseName", "inventoryMoney", "siCount", "iuCount","grossCount"]
        });
        ...
        me.__inventoryGrid = Ext.create("Ext.grid.Panel", {
            ...
            columns: [
                ...
                {header: "低于安全库存商品种类数", dataIndex: "siCount", width: 60, 
                ...
                /*
                {header: "超过库存上限的商品种类数", dataIndex: "iuCount", width: 60, 
                		menuDisabled: true, sortable: false, align: "right", 
                		xtype: "numbercolumn", format: "0",
                		renderer: function (value) {
                            return value > 0 ? "<span style='color:red'>" + value + "</span>" : value;
                ...
                */
                {header: "负毛率销售产品数", dataIndex: "grossCount", width: 60, 
                        menuDisabled: true, sortable: false, align: "right", 
                        xtype: "numbercolumn", format: "0",
                        renderer: function (value) {
                            return value > 0 ? "<span style='color:red'>" + value + "</span>" : value;
    ...
    queryInventoryData: function() {
        ...
        Ext.Ajax.request({
            url: ERP.Const.BASE_URL + "Home/Portal/inventoryPortal",
            method: "POST",
    ...
    querySaleData: function() {
        ...
        el.mask(ERP.Const.LOADING);
        Ext.Ajax.request({
            url: ERP.Const.BASE_URL + "Home/Portal/salePortal",
            method: "POST",
            callback: function (options, success, response) {
                var store = grid.getStore();
                store.removeAll();
    ...
    queryPurchaseData: function() {
        ...
        el.mask(ERP.Const.LOADING);
        Ext.Ajax.request({
            url: ERP.Const.BASE_URL + "Home/Portal/purchasePortal",
            method: "POST",
            callback: function (options, success, response) {
                var store = grid.getStore();
                store.removeAll();
    ...
    queryMoneyData: function() {
        ...
        el.mask(ERP.Const.LOADING);
        Ext.Ajax.request({
            url: ERP.Const.BASE_URL + "Home/Portal/moneyPortal",
            method: "POST",
            callback: function (options, success, response) {
                var store = grid.getStore();
                store.removeAll();
