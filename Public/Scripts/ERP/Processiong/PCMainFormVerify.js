// 加工入库 - 主界面
Ext.define("ERP.Purchase.PCMainFormVerify", {
    ...
    initComponent: function () {
        ...
        var storePCBill = Ext.create("Ext.data.Store", {
            ...
            proxy: {
                ...
                url: ERP.Const.BASE_URL + "Home/Processiong/pcBillList?type=verify",
                reader: {
                    root: 'dataList',
        ...
        var toolbar = [
                ...
                */
                {
                    text: "驳回", iconCls: "ERP-button-delete", scope: me, handler: me.onRejectPCBill
                }, "-", 

                {
                    text: "审核入库", iconCls: "ERP-button-commit", scope: me, handler: me.onCommit
                }, "-", {
    ...
    onRejectPCBill:function(){
        var me = this;
        var item = me.pcBillGrid.getSelectionModel().getSelection();
        if (item == null || item.length != 1) {
            ERP.MsgBox.showInfo("请选择要驳回的加工单");
            return;
        }
        ...
        //判断加工单的状态，看是否可以编辑
        if(pcBill.get("billStatus") == "审核中"){
            //ERP.MsgBox.showInfo("加工单正在审核中，无法驳回");
           //return;
        }
        if(pcBill.get("billStatus") == "已审核"){
            ERP.MsgBox.showInfo("加工单已经审核通过了，无法驳回");
            return;
        }
        if(pcBill.get("billStatus") == "已完成"){
            ERP.MsgBox.showInfo("加工单已经入库，无法驳回");
            return;
        }
        ...
        var info = "请确认是否驳回加工单: <span style='color:red'>" + pcBill.get("ref") + "</span>";
        var me = this;
        ERP.MsgBox.confirm(info, function () {
            var el = Ext.getBody();
            el.mask("正在驳回中...");
            Ext.Ajax.request({
                url: ERP.Const.BASE_URL + "Home/Processiong/rejectPCBill",
                method: "POST",
    ...
    onCommit: function () {
        ...
        ERP.MsgBox.confirm(info, function () {
            ...
            el.mask("正在提交中...");
            Ext.Ajax.request({
                url: ERP.Const.BASE_URL + "Home/Processiong/commitPCBill",
                method: "POST",
                params: {id: pcBill.get("id")},
                callback: function (options, success, response) {
                    el.unmask();
