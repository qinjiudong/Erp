 ...
 * @author 李静波
 */
class SupplierService extends ERPBaseService {
	...
	public function supplierList($params) {
		...
		$sql = "select id, category_id, code, name, contact01, qq01, tel01, mobile01, 
				contact02, qq02, tel02, mobile02, init_payables, init_payables_dt, period,mode,
	...
	public function editSupplier($params) {
		...
		if ($id) {
			...
			$sql = "update t_supplier 
					...
					contact02 = '%s', qq02 = '%s', tel02 = '%s', mobile02 = '%s',period = '%s',
					address = '%s', address_shipping = '%s', mode = %d
					where id = '%s'  ";
			
			$db->execute($sql, $code, $name, $categoryId, $py, $contact01, $qq01, $tel01, $mobile01, 
					$contact02, $qq02, $tel02, $mobile02,$period, $address, $addressShipping, $mode, $id);
			...
			$sql = "insert into t_supplier (id, category_id, code, name, py, contact01, 
					...
					tel02, mobile02,period, address, address_shipping, mode) 
					values ('%s', '%s', '%s', '%s', '%s', '%s', '%s', '%s', '%s', '%s', '%s', '%s',
							'%s', '%s', '%s', '%s', '%s')  ";
			$db->execute($sql, $id, $categoryId, $code, $name, $py, $contact01, $qq01, $tel01, 
					$mobile01, $contact02, $qq02, $tel02, $mobile02,$period, $address, $addressShipping, $mode);
	...
	public function supplierInfo($params) {
		...
		$sql = "select category_id, code, name, contact01, qq01, mobile01, tel01,
					contact02, qq02, mobile02, tel02, address, period, address_shipping, 
					init_payables, init_payables_dt, mode 
				from t_supplier
				where id = '%s' ";
		$data = $db->query($sql, $id);
		if ($data) {
			...
			$result["mode"] = (string)$data[0]["mode"];
